...
package body SRQ_Resolution_Machine_Tests is
   ...
   begin
      Assert
        (Transition (Identify_Source, Data_Received) = Route_SRQ,
   ...
   begin
      Assert
        (Transition (Route_SRQ, SRQ_Line_Cleared) = End_State,
         "Route_SRQ --SRQ_Line_Cleared--> End_State");
   end Test_Route_SRQ_SRQ_Line_Cleared;

   procedure Test_Identify_Source_Timeout_Try_Next_Address
     (T : in out Test_Case'Class)
   is
      pragma Unreferenced (T);
   begin
      Assert
        (Transition (Identify_Source, Timeout_Try_Next_Address)
         = Identify_Source,
   ...
   begin
      Register_Routine
        (T,
         Test_Identify_Source_Data_Received'Access,
         "Identify_Source --Data_Received--> Route_SRQ");
      Register_Routine
        (T,
         Test_Route_SRQ_SRQ_Line_Cleared'Access,
         "Route_SRQ --SRQ_Line_Cleared--> End_State");
      Register_Routine
        (T,
         Test_Identify_Source_Timeout_Try_Next_Address'Access,
         "Identify_Source --Timeout_Try_Next_Address--> Identify_Source");
      null;
   end Register_Tests;
