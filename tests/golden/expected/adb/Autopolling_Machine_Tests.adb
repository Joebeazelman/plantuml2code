...
package body Autopolling_Machine_Tests is
   ...
   procedure Test_Select_Next_Target_Target_Selected
     (T : in out Test_Case'Class)
   is
      pragma Unreferenced (T);
   begin
      Assert
        (Transition (Select_Next_Target, Target_Selected) = Send_Poll_Command,
         "Select_Next_Target --Target_Selected--> Send_Poll_Command");
   end Test_Select_Next_Target_Target_Selected;

   procedure Test_Send_Poll_Command_Command_Sent (T : in out Test_Case'Class)
   is
      pragma Unreferenced (T);
   begin
      Assert
        (Transition (Send_Poll_Command, Command_Sent) = Await_Poll_Response,
         "Send_Poll_Command --Command_Sent--> Await_Poll_Response");
   end Test_Send_Poll_Command_Command_Sent;

   procedure Test_Await_Poll_Response_Payload_Received
     (T : in out Test_Case'Class)
   is
      pragma Unreferenced (T);
   begin
      Assert
        (Transition (Await_Poll_Response, Payload_Received) = Route_Poll_Data,
         "Await_Poll_Response --Payload_Received--> Route_Poll_Data");
   end Test_Await_Poll_Response_Payload_Received;

   procedure Test_Await_Poll_Response_Timeout_Normal_No_Data
     (T : in out Test_Case'Class)
   is
      pragma Unreferenced (T);
   begin
      Assert
        (Transition (Await_Poll_Response, Timeout_Normal_No_Data) = End_State,
   ...
   begin
      Register_Routine
        (T,
         Test_Select_Next_Target_Target_Selected'Access,
         "Select_Next_Target --Target_Selected--> Send_Poll_Command");
      Register_Routine
        (T,
         Test_Send_Poll_Command_Command_Sent'Access,
         "Send_Poll_Command --Command_Sent--> Await_Poll_Response");
      Register_Routine
        (T,
         Test_Await_Poll_Response_Payload_Received'Access,
         "Await_Poll_Response --Payload_Received--> Route_Poll_Data");
      Register_Routine
        (T,
         Test_Await_Poll_Response_Timeout_Normal_No_Data'Access,
         "Await_Poll_Response --Timeout_Normal_No_Data--> End_State");
      null;
   end Register_Tests;
