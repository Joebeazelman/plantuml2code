...
package body Machine is
      ...
      case Current_State (Self) is
         ...
         when ADB_Reset                =>
            case Via_History (Self) is
               when History_None    =>
                  ADB_Reset_Machine.Base.Reset (Self.ADB_Reset_Child);
         ...
         when Autopolling              =>
            case Via_History (Self) is
               when History_None    =>
                  Autopolling_Machine.Base.Reset (Self.Autopolling_Child);
   ...
   procedure Step_ADB_Reset
     (Self : in out Machine; On : ADB_Reset_Machine.Event) is
   begin
   ...
   procedure Step_Address_Resolution
     (Self : in out Machine; On : Address_Resolution_Machine.Event) is
   begin
      if Current_State (Self) = Address_Resolution then
         Address_Resolution_Machine.Base.Step
           (Self.Address_Resolution_Child, On);
      end if;
   end Step_Address_Resolution;

   function Address_Resolution_State
     (Self : Machine) return Address_Resolution_Machine.State is
   begin
      return
        Address_Resolution_Machine.Base.Current_State
          (Self.Address_Resolution_Child);
   end Address_Resolution_State;

   procedure Step_Execute_Explicit_Command
     (Self : in out Machine; On : Execute_Explicit_Command_Machine.Event) is
   begin
      if Current_State (Self) = Execute_Explicit_Command then
         Execute_Explicit_Command_Machine.Base.Step
           (Self.Execute_Explicit_Command_Child, On);
      end if;
   end Step_Execute_Explicit_Command;

   function Execute_Explicit_Command_State
     (Self : Machine) return Execute_Explicit_Command_Machine.State is
   begin
      return
        Execute_Explicit_Command_Machine.Base.Current_State
          (Self.Execute_Explicit_Command_Child);
   end Execute_Explicit_Command_State;

   procedure Step_Autopolling
     (Self : in out Machine; On : Autopolling_Machine.Event) is
   begin
   ...
   procedure Step_SRQ_Resolution
     (Self : in out Machine; On : SRQ_Resolution_Machine.Event) is
   begin
   ...
   function SRQ_Resolution_State
     (Self : Machine) return SRQ_Resolution_Machine.State is
   begin
      return
        SRQ_Resolution_Machine.Base.Current_State (Self.SRQ_Resolution_Child);
