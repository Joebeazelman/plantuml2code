...
package State_Machine.Machines is
   ...
   function Next_State (Self : Machine; On : Event) return State is abstract;
   ...
   with
     Pre  => not Is_Terminated (Self),
     Post =>
       (if Current_State (Self) = Current_State (Self)'Old
        then Is_Terminated (Self) = Is_Terminated (Self)'Old);

   procedure Reset (Self : in out Machine'Class)
   with
     Post => Current_State (Self) = Initial and then not Is_Terminated (Self);
