...
package body Nested_Tests is
   ...
   begin
      Assert (Transition (Idle, Start) = Running, "Idle --Start--> Running");
   end Test_Idle_Start;
   ...
   begin
      Assert
        (Transition (Idle, Continue) = Running, "Idle --Continue--> Running");
   end Test_Idle_Continue;

   procedure Test_Running_Stop (T : in out Test_Case'Class) is
      pragma Unreferenced (T);
   begin
      Assert (Transition (Running, Stop) = Idle, "Running --Stop--> Idle");
   ...
   begin
      Assert
        (Transition (Running, Finish) = End_State,
   ...
   begin
      Register_Routine (T, Test_Idle_Start'Access, "Idle --Start--> Running");
      Register_Routine
        (T, Test_Idle_Continue'Access, "Idle --Continue--> Running");
      Register_Routine (T, Test_Running_Stop'Access, "Running --Stop--> Idle");
      Register_Routine
        (T, Test_Running_Finish'Access, "Running --Finish--> End_State");
      null;
   end Register_Tests;
