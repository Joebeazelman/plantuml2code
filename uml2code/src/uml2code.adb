--  uml2code -- parse PlantUML diagrams and emit them in a
--  chosen format.
--
--  This file is only the orchestration layer. Argument parsing lives
--  in Uml2Code_CLI, help text in Uml2Code_Help, and
--  command bodies in Uml2Code_Commands.
...
procedure Uml2Code is
   ...
   Args : constant Parse_Result := Uml2Code_CLI.Parse;
...
begin
   Uml2Code_Ansi.Set_Mode (Args.Color);
   ...
   case Args.Cmd is
      when Cmd_Help =>
         Uml2Code_Help.Print_Header (Standard_Output);
         New_Line;
         if Length (Args.Help_Topic) > 0 then
            begin
               Uml2Code_Help.Print_Topic
            ...
            Uml2Code_Help.Print_Usage (Standard_Output);
         end if;
      ...
      when Cmd_Version =>
         Put_Line (Standard_Output,
                   Program_Name & " "
                   & Uml2code_Config.Crate_Version);
         Set_Exit_Status (Success);
         return;

      when Cmd_None =>
         Fail ("no command given");
         Uml2Code_Help.Print_Usage (Standard_Error);
         Set_Exit_Status (Failure);
         return;
   ...
   if Length (Args.Templates_Dir) > 0 then
      ...
      begin
         ...
         Uml2Code_Template_Path.Set_Override (D);
      end;
   end if;
   ...
   if Length (Args.Templates_Dir) = 0 then
      begin
         Uml2Code_Template_Path.Load_Config;
      exception
         when Uml2Code_Template_Path.Config_Error =>
      ...
      begin
         begin
            case Args.Cmd is
               ...
               when Cmd_Kind =>
                  if not Uml2Code_Commands.Run_Kind (Path) then
                     Ok := False;
                  end if;
               when others =>
                  null;
         ...
         exception
            ...
            when Uml2Code_Template_Path.Template_Not_Found =>
